...
function SaleChange(Sale){
    for (let index = 0; index < 5; index++){
        let saleEl;
        saleEl = document.getElementById("sale"+(index+1));
        saleEl.textContent = Sale[index];
    }
}
...
function PriceChange(Price){
    for (let index = 0; index < 5; index++){
        let priceEl;
        priceEl = document.getElementById("price"+(index+1));
...
function women(){
    let imgSpan = ['꽈배기 베이지 니트', '오버핏 셔츠', '베이지 주름 치마', '크롭 무지 티셔츠', '여성 자켓'];
